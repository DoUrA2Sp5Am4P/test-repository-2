...
func getExpressions(w http.ResponseWriter, r *http.Request) {
	...
	current_user_expressions := []Expression{}
	for i := 0; i < len(expressions); i++ {
		if expressions[i].Username == currentUser {
			current_user_expressions = append(current_user_expressions, expressions[i])
		}
	}
	json.NewEncoder(w).Encode(current_user_expressions)
...
func addExpression(w http.ResponseWriter, r *http.Request) {
	...
	expression.Username = currentUser
